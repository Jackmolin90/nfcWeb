        ...
        <div class="content-wrapper">
          <div class="top-wrapper">
            <div class="left-wrapper">
              <span>{{ $t('lock.pledge') }}</span>
              <span>{{ $t('lock.currentPledgeNumber',[total]) }}</span>
            </div>
            <div class="search display-pc-and-pad">
              <div>
                <el-input v-model="searchInput" :placeholder="$t('lock.pledgeSearch')" @keyup.enter.native="seachData()" />
              </div>
          ...
          <div class="list-wrapper">
            <div class="display-pc-and-pad">
              <el-table :data="tableData" :cell-style="cellStyle" stripe style="width: 100%">
                ...
                <el-table-column :label="$t('lock.pledgeType')" width="200">
                  <template slot-scope="scope">
                    <span v-if="scope.row.status == 2"> CN {{ $t('lock.node') }} </span>
                    <span v-else-if="scope.row.status == 3"> EN {{ $t('lock.node') }} </span>
                    <span v-else-if="scope.row.status == 1">RN {{ $t('lock.node') }} </span>
                  </template>
                </el-table-column>
                <el-table-column :label="$t('lock.pledgeAddress')" width="690">
                  <template slot-scope="scope">
                    <span style="color:#4A7BD3"> {{ scope.row.pledgeAddress }} </span>
                  </template>
                </el-table-column>
                <el-table-column :label="$t('lock.totalPledgeAmount')" prop="totalPledgeAmount" width="200"/>
              </el-table>
            </div>
            <div class="display-phone">
              ...
              <div v-for="(item, index) in tableData" :key="index" class="item-wrapper" >
                <div class="addr">
                  <div>{{ item.order }}</div>
                  <div>
                    <span style="color:#4A7BD3"> {{ getAddrFormat(item.pledgeAddress,12) }} </span>
                  </div>
                </div>
                <div class="other">
                  <div>
                    <span>{{ $t('lock.pledgeType') }}</span>
                    <span v-if="item.status == 2"> CN {{ $t('lock.node') }} </span>
                    <span v-else-if="item.status == 3"> EN {{ $t('lock.node') }} </span>
                    <span v-else-if="item.status == 1">RN {{ $t('lock.node') }} </span>
                  </div>
                  <div>
                    <span>{{ $t('lock.totalPledgeAmount') }}</span>
...
export default {
  ...
  methods: {
    ...
    getAddrList(page, device = 0) {
      ...
      api.postJson('/platform/lockAndPledgeInfo', param).then(response => {
