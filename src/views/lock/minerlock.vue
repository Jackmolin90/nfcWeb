      ...
      <div class="block-container">
        <div class="content-wrapper">
          ...
          <div class="list-wrapper">
            <div class="display-pc-and-pad">
              ...
              >
                ...
                <el-table-column
                  v-if="false"
                  :label="$t('lock.profitaddress')"
                  prop="revenue_address"
                  show-overflow-tooltip
                />
                <el-table-column
                  :label="$t('lock.totalPledgeAmount')"
                  prop="pledge_amount"
                  width="200"
                />
                <el-table-column
                ...
                <el-table-column :label="$t('lock.state')" min-width="80">
                  <template slot-scope="scope">
                    {{ $t(scope.row.state) }}
                  </template>
                  <!-- <template slot-scope="scope">
          ...
          <div
            ...
            @click="getMinerLockList(page + 1, 1)"
          >
            {{ loadText }}
          </div>
        </div>
...
export default {
  ...
  methods: {
    toviews(data) {
      if (data.address) {
        helper.goPathUrl(this, "topminingview", data.address);
      }
    },
    toRelease(data) {
    ...
    async getMinerLockList(page, device = 0) {
        ...
        .then((response) => {
          if (response.statusCode !== 0) {
            ...
            data.forEach((item, index) => {
              ...
              let state = totalAmount == "0" ? "lock.stateOk" : "lock.Waiting";
