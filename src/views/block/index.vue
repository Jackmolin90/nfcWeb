        ...
        <div class="content-wrapper">
          <div class="top-wrapper">
            <div class="left-wrapper">
              ...
              <span>{{ $t('home.currentBlockNumber',[total]) }}</span>
            </div>
            <div class="page-wrapper">
              <span :class="page > 1 ? 'cursor' : 'gray' " class="space" @click="getBlockList(1)">{{ $t('comm.firstPage') }}</span>
              <span :class="page > 1 ? 'page-cursor' : 'gray' " class="space symbol" @click="getBlockList(page-1)"> &lt; </span>
              <span class="forcibly-blue">{{ page }}</span>/<span class="space forcibly-blue">{{ totalPages }}</span>
              <span :class="page == totalPages ? 'gray' : 'page-cursor' " class="space symbol" @click="getBlockList(page+1)"> &gt; </span>
              <span :class="page == totalPages ? 'gray' : 'cursor' " @click="getBlockList(totalPages)">{{ $t('comm.lastPage') }}</span>
              <!-- <span class="span-goto">{{ $t("comm.goto")}}</span>
              <span><el-input class="input-page" v-model="currPage" @keyup.enter.native="getBlockList(currPage)" ></el-input></span>
              <span>{{ $t("comm.page")}}</span> -->
            </div>
          </div>
          <div class="list-wrapper">
            <div v-for="(item, index) in blockList" :key="index" class="item-wrapper">
              <div class="content">
                <div class="item">
                  <div>
                    <h2 class="cursor" @click="goToUrl('Blockinfo',item.block)">{{ item.block }}</h2>
                  </div>
                  <div class="time"> {{ formatTime(item.time) }}{{ false?$t('comm.before'):"" }}</div>
                </div>
                <div class="item">
                  <div>
                ...
                <div class="item">
                  ...
                  <div class="node">
                    {{ item.validation }}
                    <span >
                      <span class="display-pc cursor equal-width" @click="goToUrl('Accountsinfo',item.validationAddress)">{{ getAddrFormat(item.validationAddress,10) }}</span>
                      <span class="display-pad cursor equal-width" @click="goToUrl('Accountsinfo',item.validationAddress)">{{ getAddrFormat(item.validationAddress,10) }}</span>
                    </span>
                  </div>
                </div>
                <div class="item">
                  ...
                  <div>
                    {{ item.gasUsed }} ({{ item.gasUsedRate }}%)
                    <el-slider v-model="item.gasUsedRate" disabled />
                  </div>
                </div>
                <div class="item">
                  <div>
          ...
          <div class="bottom-wrapper">
            <div class="left-wrapper">
              ...
              <span>
                <el-select v-model="prePage" :placeholder="$t('comm.selected')" size="mini" @change="getBlockList(page)">
        ...
        <div class="block-swiper">
          ...
          <div class="content-wrapper">
            ...
            <div v-for="(item, index) in blockList" :key="index" class="card-wrapper">
              <div class="item">
                <div class="content">
                  <div class="block-height cursor " @click="goToUrl('Blockinfo',item.block)">
                    {{ item.block }}
                  </div>
                  <div class="other">
                    <p>{{ $t('home.tranNumber',[item.transition]) }}</p>
                    <p>{{ $t('home.verificationNode') }}<span class="node cursor" @click="goToUrl('accountsinfo',item.validationAddress)">
                      {{ item.validation }}({{ getAddrFormat(item.validationAddress,12) }})
                      </span></p>
                    <p class="block-reward">{{ $t('home.blockReward') }}
                      <span>
            ...
            <div class="load-more" @click="getBlockList(page+1,1)">
              {{ $t('comm.clickLoad') }}
            </div>
          </div>
        </div>
        ...
        if (response.statusCode !== 0) {
          ...
          data.forEach(item => {
            temp = {
              ...
              gasUsedRate: Math.floor(item.gasUsed / item.gasLimit * 10000) / 100,
